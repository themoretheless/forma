...
fn main() {
    ...
    let source = args.get(1).map(|p| std::fs::read_to_string(p).unwrap()).unwrap_or(forma_vector::EXAMPLE.into());
    let template = args.get(2).map(|p| std::fs::read_to_string(p).unwrap()).unwrap_or(forma_vector::BUTTON_COMPONENT.into());
    for (w,h,scale) in [(400,200,1.),(800,400,2.),(1600,1000,2.)] {
        let mut b=forma_vector::Button::from_sources(&source,&template).unwrap();
